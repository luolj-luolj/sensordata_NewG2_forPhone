...
public final class USBMonitor {
	...
	public synchronized void register() throws IllegalStateException {
		...
		if (mPermissionIntent == null) {
			...
			mDeviceCounts = 0;
			mAsyncHandler.postDelayed(mDeviceCheckRunnable, 0);
		}
	}
...
//		if (DEBUG) Log.v(TAG, "requestPermission:device=" + device);
	...
	private final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
		...
		public void onReceive(final Context context, final Intent intent) {
			...
			if (DEBUG) Log.v("PhoneCursor", "BroadcastReceiver action = " + action);
	...
	private final Runnable mDeviceCheckRunnable = new Runnable() {
		...
		public void run() {
			...
			if ((n > mDeviceCounts) || (m > hasPermissionCounts)) {
				...
				if (mOnDeviceConnectListener != null) {
					for (int i = 0; i < n; i++) {
						...
						if (DEBUG) Log.v(TAG, String.format("mDeviceCheckRunnable: %d/%d", i , n));
			...
			mAsyncHandler.postDelayed(this, 0);
		}
	};
